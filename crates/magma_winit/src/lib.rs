...
```
# use std::error::Error;
# use magma_app::{magma_ecs::ComponentStore, App, schedule::Update, World};
...
fn close_windows(component_store: &mut ComponentStore) {
    for index in component_store
        ...
        .collect::<Vec<usize>>()
    {
        component_store.delete_entity(index);
    }
}
```
*/
...
use magma_app::magma_ecs::query::Query;
use magma_app::magma_ecs::world::UnsafeWorldMut;
...
impl Module for WinitModule {
    fn init(self, app: &mut magma_app::App) {
        ...
        app.add_system(PostUpdate, winit_update_changed_windows)
            .unwrap();
        app.add_system(PostUpdate, winit_drop_windows).unwrap();
        app.world
            .component_store
            .register_component::<CachedWindow>();
    }
}
...
impl ApplicationHandler for WinitAppState {
    ...
    fn window_event(
        ...
        // SAFETY: All data retrieved through unsafe is not retrieved again until dropped.
        let Some(window) = unsafe { UnsafeWorldMut::new(&mut self.app.world).get_mut() }
            .component_store
        ...
        let Some(cached_window) = self
            .app
            .world
            .component_store
            .get_components_mut::<CachedWindow>()
            .unwrap()
            .get_mut(window_entity)
        else {
            return;
        ...
        match event {
            WindowEvent::Resized(physical_size) => {
                let res = WindowResolution::new(physical_size.width, physical_size.height);
                window.resolution = res;
                cached_window.window.resolution = res;
                self.app
                    .world
                    .event_manager
            ...
            WindowEvent::Moved(physical_position) => {
                let pos = magma_windowing::window::WindowPosition::Pos(IVec2::new(
                    physical_position.x,
                    physical_position.y,
                ));
                window.position = pos;
                cached_window.window.position = pos;
                self.app
                    .world
                    .event_manager
            ...
            WindowEvent::Focused(focus) => {
                ...
                cached_window.window.focused = focus;
                self.app
                    .world
                    .event_manager
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                window.cursor_position = pos;
                cached_window.window.cursor_position = pos;
                self.app
                    .world
                    .event_manager
            ...
            WindowEvent::ThemeChanged(theme) => {
                ...
                window.window_theme = theme;
                cached_window.window.window_theme = theme;
                self.app
                    .world
                    .event_manager
            ...
            WindowEvent::KeyboardInput { event, .. } => self
                ...
                .send_event(KeyboardInput {
                    ...
                    key_code: map_key_code(event.physical_key),
    ...
    fn about_to_wait(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        ...
        // update the app
        self.app.run_schedule(PreUpdate);
        self.app.run_schedule(Update);
        self.app.run_schedule(PostUpdate);
        self.app.world.event_manager.clear();
    }
}
...
impl WinitAppState {
    pub fn winit_update(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let winit_windows = unsafe { unsafe_world.get_mut() }
            .resource_store
            .get_mut::<Windows>()
            .unwrap();
        // create winit windows for new window components
        for (entity, cached_window) in Query::<&Window>::new(unsafe_world)
            .data
            .iter()
            .filter_map(|(entity, window)| {
                if !winit_windows.entity_to_window.contains_key(&entity) {
                    winit_windows.create_winit_window(&self.app.world, event_loop, window, entity);
                    self.app
                        .world
                        .event_manager
        ...
        // remove deleted windows from winit
        let windows = self
            .app
            .world
            .component_store
            .get_components_ref::<Window>()
            .unwrap();
        for entity in winit_windows
            .window_to_entity
            .values()
            .copied()
            .collect::<Vec<_>>()
        {
            if windows.get(entity).is_none() {
                self.app
                    .world
                    .resource_store
                    .get_mut::<WindowsToDrop>()
                    .unwrap()
                    .0
                    .push(winit_windows.remove_window(entity).unwrap());
                self.app
                    .world
                    .event_manager
                    .send_event(WindowClosed { window: entity })
                    .unwrap();
            }
        }

        // exit if no windows are present
        if winit_windows.winit_windows.is_empty() {
            event_loop.exit();
        }
    }
}
...
fn map_key_code(key: winit::keyboard::PhysicalKey) -> magma_input::keyboard::KeyCode {
    match key {
        winit::keyboard::PhysicalKey::Code(key_code) => match key_code {
            winit::keyboard::KeyCode::Backquote => magma_input::keyboard::KeyCode::Backquote,
            winit::keyboard::KeyCode::Backslash => magma_input::keyboard::KeyCode::Backslash,
            winit::keyboard::KeyCode::BracketLeft => magma_input::keyboard::KeyCode::BracketLeft,
            winit::keyboard::KeyCode::BracketRight => magma_input::keyboard::KeyCode::BracketRight,
            winit::keyboard::KeyCode::Comma => magma_input::keyboard::KeyCode::Comma,
            winit::keyboard::KeyCode::Digit0 => magma_input::keyboard::KeyCode::Digit0,
            winit::keyboard::KeyCode::Digit1 => magma_input::keyboard::KeyCode::Digit1,
            winit::keyboard::KeyCode::Digit2 => magma_input::keyboard::KeyCode::Digit2,
            winit::keyboard::KeyCode::Digit3 => magma_input::keyboard::KeyCode::Digit3,
            winit::keyboard::KeyCode::Digit4 => magma_input::keyboard::KeyCode::Digit4,
            winit::keyboard::KeyCode::Digit5 => magma_input::keyboard::KeyCode::Digit5,
            winit::keyboard::KeyCode::Digit6 => magma_input::keyboard::KeyCode::Digit6,
            winit::keyboard::KeyCode::Digit7 => magma_input::keyboard::KeyCode::Digit7,
            winit::keyboard::KeyCode::Digit8 => magma_input::keyboard::KeyCode::Digit8,
            winit::keyboard::KeyCode::Digit9 => magma_input::keyboard::KeyCode::Digit9,
            winit::keyboard::KeyCode::Equal => magma_input::keyboard::KeyCode::Equal,
            winit::keyboard::KeyCode::IntlBackslash => {
                magma_input::keyboard::KeyCode::IntlBackslash
            }
            winit::keyboard::KeyCode::IntlRo => magma_input::keyboard::KeyCode::IntlRo,
            winit::keyboard::KeyCode::IntlYen => magma_input::keyboard::KeyCode::IntlYen,
            ...
            winit::keyboard::KeyCode::Minus => magma_input::keyboard::KeyCode::Minus,
            winit::keyboard::KeyCode::Period => magma_input::keyboard::KeyCode::Period,
            winit::keyboard::KeyCode::Quote => magma_input::keyboard::KeyCode::Quote,
            winit::keyboard::KeyCode::Semicolon => magma_input::keyboard::KeyCode::Semicolon,
            winit::keyboard::KeyCode::Slash => magma_input::keyboard::KeyCode::Slash,
            winit::keyboard::KeyCode::AltLeft => magma_input::keyboard::KeyCode::AltLeft,
            winit::keyboard::KeyCode::AltRight => magma_input::keyboard::KeyCode::AltRight,
            winit::keyboard::KeyCode::Backspace => magma_input::keyboard::KeyCode::Backspace,
            winit::keyboard::KeyCode::CapsLock => magma_input::keyboard::KeyCode::CapsLock,
            winit::keyboard::KeyCode::ContextMenu => magma_input::keyboard::KeyCode::ContextMenu,
            winit::keyboard::KeyCode::ControlLeft => magma_input::keyboard::KeyCode::ControlLeft,
            winit::keyboard::KeyCode::ControlRight => magma_input::keyboard::KeyCode::ControlRight,
            winit::keyboard::KeyCode::Enter => magma_input::keyboard::KeyCode::Enter,
            winit::keyboard::KeyCode::SuperLeft => magma_input::keyboard::KeyCode::SuperLeft,
            winit::keyboard::KeyCode::SuperRight => magma_input::keyboard::KeyCode::SuperRight,
            winit::keyboard::KeyCode::ShiftLeft => magma_input::keyboard::KeyCode::ShiftLeft,
            winit::keyboard::KeyCode::ShiftRight => magma_input::keyboard::KeyCode::ShiftRight,
            winit::keyboard::KeyCode::Space => magma_input::keyboard::KeyCode::Space,
            winit::keyboard::KeyCode::Tab => magma_input::keyboard::KeyCode::Tab,
            winit::keyboard::KeyCode::Convert => magma_input::keyboard::KeyCode::Convert,
            winit::keyboard::KeyCode::KanaMode => magma_input::keyboard::KeyCode::KanaMode,
            winit::keyboard::KeyCode::Lang1 => magma_input::keyboard::KeyCode::Lang1,
            winit::keyboard::KeyCode::Lang2 => magma_input::keyboard::KeyCode::Lang2,
            winit::keyboard::KeyCode::Lang3 => magma_input::keyboard::KeyCode::Lang3,
            winit::keyboard::KeyCode::Lang4 => magma_input::keyboard::KeyCode::Lang4,
            winit::keyboard::KeyCode::Lang5 => magma_input::keyboard::KeyCode::Lang5,
            winit::keyboard::KeyCode::NonConvert => magma_input::keyboard::KeyCode::NonConvert,
            winit::keyboard::KeyCode::Delete => magma_input::keyboard::KeyCode::Delete,
            ...
            winit::keyboard::KeyCode::Insert => magma_input::keyboard::KeyCode::Insert,
            winit::keyboard::KeyCode::PageDown => magma_input::keyboard::KeyCode::PageDown,
            winit::keyboard::KeyCode::PageUp => magma_input::keyboard::KeyCode::PageUp,
            winit::keyboard::KeyCode::ArrowDown => magma_input::keyboard::KeyCode::ArrowDown,
            winit::keyboard::KeyCode::ArrowLeft => magma_input::keyboard::KeyCode::ArrowLeft,
            winit::keyboard::KeyCode::ArrowRight => magma_input::keyboard::KeyCode::ArrowRight,
            winit::keyboard::KeyCode::ArrowUp => magma_input::keyboard::KeyCode::ArrowUp,
            winit::keyboard::KeyCode::NumLock => magma_input::keyboard::KeyCode::NumLock,
            winit::keyboard::KeyCode::Numpad0 => magma_input::keyboard::KeyCode::Numpad0,
            winit::keyboard::KeyCode::Numpad1 => magma_input::keyboard::KeyCode::Numpad1,
            winit::keyboard::KeyCode::Numpad2 => magma_input::keyboard::KeyCode::Numpad2,
            winit::keyboard::KeyCode::Numpad3 => magma_input::keyboard::KeyCode::Numpad3,
            winit::keyboard::KeyCode::Numpad4 => magma_input::keyboard::KeyCode::Numpad4,
            winit::keyboard::KeyCode::Numpad5 => magma_input::keyboard::KeyCode::Numpad5,
            winit::keyboard::KeyCode::Numpad6 => magma_input::keyboard::KeyCode::Numpad6,
            winit::keyboard::KeyCode::Numpad7 => magma_input::keyboard::KeyCode::Numpad7,
            winit::keyboard::KeyCode::Numpad8 => magma_input::keyboard::KeyCode::Numpad8,
            winit::keyboard::KeyCode::Numpad9 => magma_input::keyboard::KeyCode::Numpad9,
            winit::keyboard::KeyCode::NumpadAdd => magma_input::keyboard::KeyCode::NumpadAdd,
            winit::keyboard::KeyCode::NumpadBackspace => {
                magma_input::keyboard::KeyCode::NumpadBackspace
            }
            winit::keyboard::KeyCode::NumpadClear => magma_input::keyboard::KeyCode::NumpadClear,
            winit::keyboard::KeyCode::NumpadClearEntry => {
                magma_input::keyboard::KeyCode::NumpadClearEntry
            }
            winit::keyboard::KeyCode::NumpadComma => magma_input::keyboard::KeyCode::NumpadComma,
            winit::keyboard::KeyCode::NumpadDecimal => {
                magma_input::keyboard::KeyCode::NumpadDecimal
            }
            winit::keyboard::KeyCode::NumpadDivide => magma_input::keyboard::KeyCode::NumpadDivide,
            winit::keyboard::KeyCode::NumpadEnter => magma_input::keyboard::KeyCode::NumpadEnter,
            winit::keyboard::KeyCode::NumpadEqual => magma_input::keyboard::KeyCode::NumpadEqual,
            winit::keyboard::KeyCode::NumpadHash => magma_input::keyboard::KeyCode::NumpadHash,
            ...
            winit::keyboard::KeyCode::NumpadStar => magma_input::keyboard::KeyCode::NumpadStar,
            winit::keyboard::KeyCode::NumpadSubtract => {
                magma_input::keyboard::KeyCode::NumpadSubtract
            }
            winit::keyboard::KeyCode::Escape => magma_input::keyboard::KeyCode::Escape,
            winit::keyboard::KeyCode::Fn => magma_input::keyboard::KeyCode::Fn,
            winit::keyboard::KeyCode::FnLock => magma_input::keyboard::KeyCode::FnLock,
            winit::keyboard::KeyCode::PrintScreen => magma_input::keyboard::KeyCode::PrintScreen,
            winit::keyboard::KeyCode::ScrollLock => magma_input::keyboard::KeyCode::ScrollLock,
            winit::keyboard::KeyCode::Pause => magma_input::keyboard::KeyCode::Pause,
            winit::keyboard::KeyCode::BrowserBack => magma_input::keyboard::KeyCode::BrowserBack,
            ...
            winit::keyboard::KeyCode::BrowserHome => magma_input::keyboard::KeyCode::BrowserHome,
            ...
            winit::keyboard::KeyCode::BrowserStop => magma_input::keyboard::KeyCode::BrowserStop,
            winit::keyboard::KeyCode::Eject => magma_input::keyboard::KeyCode::Eject,
            winit::keyboard::KeyCode::LaunchApp1 => magma_input::keyboard::KeyCode::LaunchApp1,
            winit::keyboard::KeyCode::LaunchApp2 => magma_input::keyboard::KeyCode::LaunchApp2,
            winit::keyboard::KeyCode::LaunchMail => magma_input::keyboard::KeyCode::LaunchMail,
            winit::keyboard::KeyCode::MediaPlayPause => {
                magma_input::keyboard::KeyCode::MediaPlayPause
            }
            winit::keyboard::KeyCode::MediaSelect => magma_input::keyboard::KeyCode::MediaSelect,
            winit::keyboard::KeyCode::MediaStop => magma_input::keyboard::KeyCode::MediaStop,
            ...
            winit::keyboard::KeyCode::Power => magma_input::keyboard::KeyCode::Power,
            winit::keyboard::KeyCode::Sleep => magma_input::keyboard::KeyCode::Sleep,
            ...
            winit::keyboard::KeyCode::WakeUp => magma_input::keyboard::KeyCode::WakeUp,
            winit::keyboard::KeyCode::Meta => magma_input::keyboard::KeyCode::Meta,
            winit::keyboard::KeyCode::Hyper => magma_input::keyboard::KeyCode::Hyper,
            winit::keyboard::KeyCode::Turbo => magma_input::keyboard::KeyCode::Turbo,
            winit::keyboard::KeyCode::Abort => magma_input::keyboard::KeyCode::Abort,
            winit::keyboard::KeyCode::Resume => magma_input::keyboard::KeyCode::Resume,
            winit::keyboard::KeyCode::Suspend => magma_input::keyboard::KeyCode::Suspend,
            winit::keyboard::KeyCode::Again => magma_input::keyboard::KeyCode::Again,
            ...
            winit::keyboard::KeyCode::Paste => magma_input::keyboard::KeyCode::Paste,
            winit::keyboard::KeyCode::Props => magma_input::keyboard::KeyCode::Props,
            winit::keyboard::KeyCode::Select => magma_input::keyboard::KeyCode::Select,
            winit::keyboard::KeyCode::Undo => magma_input::keyboard::KeyCode::Undo,
            winit::keyboard::KeyCode::Hiragana => magma_input::keyboard::KeyCode::Hiragana,
            winit::keyboard::KeyCode::Katakana => magma_input::keyboard::KeyCode::Katakana,
...
fn map_key(key: winit::keyboard::Key) -> magma_input::keyboard::Key {
    match key {
        winit::keyboard::Key::Named(named_key) => match named_key {
            winit::keyboard::NamedKey::Alt => magma_input::keyboard::Key::Alt,
            winit::keyboard::NamedKey::AltGraph => magma_input::keyboard::Key::AltGraph,
            winit::keyboard::NamedKey::CapsLock => magma_input::keyboard::Key::CapsLock,
            winit::keyboard::NamedKey::Control => magma_input::keyboard::Key::Control,
            winit::keyboard::NamedKey::Fn => magma_input::keyboard::Key::Fn,
            winit::keyboard::NamedKey::FnLock => magma_input::keyboard::Key::FnLock,
            winit::keyboard::NamedKey::NumLock => magma_input::keyboard::Key::NumLock,
            winit::keyboard::NamedKey::ScrollLock => magma_input::keyboard::Key::ScrollLock,
            ...
            winit::keyboard::NamedKey::SymbolLock => magma_input::keyboard::Key::SymbolLock,
            ...
            winit::keyboard::NamedKey::ArrowDown => magma_input::keyboard::Key::ArrowDown,
            winit::keyboard::NamedKey::ArrowLeft => magma_input::keyboard::Key::ArrowLeft,
            winit::keyboard::NamedKey::ArrowRight => magma_input::keyboard::Key::ArrowRight,
            winit::keyboard::NamedKey::ArrowUp => magma_input::keyboard::Key::ArrowUp,
            winit::keyboard::NamedKey::End => magma_input::keyboard::Key::End,
            winit::keyboard::NamedKey::Home => magma_input::keyboard::Key::Home,
            winit::keyboard::NamedKey::PageDown => magma_input::keyboard::Key::PageDown,
            winit::keyboard::NamedKey::PageUp => magma_input::keyboard::Key::PageUp,
            winit::keyboard::NamedKey::Backspace => magma_input::keyboard::Key::Backspace,
            ...
            winit::keyboard::NamedKey::EraseEof => magma_input::keyboard::Key::EraseEof,
            ...
            winit::keyboard::NamedKey::ContextMenu => magma_input::keyboard::Key::ContextMenu,
            winit::keyboard::NamedKey::Escape => magma_input::keyboard::Key::Escape,
            winit::keyboard::NamedKey::Execute => magma_input::keyboard::Key::Execute,
            ...
            winit::keyboard::NamedKey::ZoomOut => magma_input::keyboard::Key::ZoomOut,
            winit::keyboard::NamedKey::BrightnessDown => magma_input::keyboard::Key::BrightnessDown,
            winit::keyboard::NamedKey::BrightnessUp => magma_input::keyboard::Key::BrightnessUp,
            ...
            winit::keyboard::NamedKey::PowerOff => magma_input::keyboard::Key::PowerOff,
            winit::keyboard::NamedKey::PrintScreen => magma_input::keyboard::Key::PrintScreen,
            winit::keyboard::NamedKey::Hibernate => magma_input::keyboard::Key::Hibernate,
            winit::keyboard::NamedKey::Standby => magma_input::keyboard::Key::Standby,
            winit::keyboard::NamedKey::WakeUp => magma_input::keyboard::Key::WakeUp,
            winit::keyboard::NamedKey::AllCandidates => magma_input::keyboard::Key::AllCandidates,
            winit::keyboard::NamedKey::Alphanumeric => magma_input::keyboard::Key::Alphanumeric,
            winit::keyboard::NamedKey::CodeInput => magma_input::keyboard::Key::CodeInput,
            winit::keyboard::NamedKey::Compose => magma_input::keyboard::Key::Compose,
            winit::keyboard::NamedKey::Convert => magma_input::keyboard::Key::Convert,
            winit::keyboard::NamedKey::FinalMode => magma_input::keyboard::Key::FinalMode,
            winit::keyboard::NamedKey::GroupFirst => magma_input::keyboard::Key::GroupFirst,
            winit::keyboard::NamedKey::GroupLast => magma_input::keyboard::Key::GroupLast,
            winit::keyboard::NamedKey::GroupNext => magma_input::keyboard::Key::GroupNext,
            winit::keyboard::NamedKey::GroupPrevious => magma_input::keyboard::Key::GroupPrevious,
            winit::keyboard::NamedKey::ModeChange => magma_input::keyboard::Key::ModeChange,
            winit::keyboard::NamedKey::NextCandidate => magma_input::keyboard::Key::NextCandidate,
            winit::keyboard::NamedKey::NonConvert => magma_input::keyboard::Key::NonConvert,
            winit::keyboard::NamedKey::PreviousCandidate => {
                magma_input::keyboard::Key::PreviousCandidate
            }
            winit::keyboard::NamedKey::Process => magma_input::keyboard::Key::Process,
            winit::keyboard::NamedKey::SingleCandidate => {
                magma_input::keyboard::Key::SingleCandidate
            }
            winit::keyboard::NamedKey::HangulMode => magma_input::keyboard::Key::HangulMode,
            winit::keyboard::NamedKey::HanjaMode => magma_input::keyboard::Key::HanjaMode,
            winit::keyboard::NamedKey::JunjaMode => magma_input::keyboard::Key::JunjaMode,
            winit::keyboard::NamedKey::Eisu => magma_input::keyboard::Key::Eisu,
            winit::keyboard::NamedKey::Hankaku => magma_input::keyboard::Key::Hankaku,
            winit::keyboard::NamedKey::Hiragana => magma_input::keyboard::Key::Hiragana,
            winit::keyboard::NamedKey::HiraganaKatakana => {
                magma_input::keyboard::Key::HiraganaKatakana
            }
            winit::keyboard::NamedKey::KanaMode => magma_input::keyboard::Key::KanaMode,
            winit::keyboard::NamedKey::KanjiMode => magma_input::keyboard::Key::KanjiMode,
            winit::keyboard::NamedKey::Katakana => magma_input::keyboard::Key::Katakana,
            winit::keyboard::NamedKey::Romaji => magma_input::keyboard::Key::Romaji,
            winit::keyboard::NamedKey::Zenkaku => magma_input::keyboard::Key::Zenkaku,
            winit::keyboard::NamedKey::ZenkakuHankaku => magma_input::keyboard::Key::ZenkakuHankaku,
            ...
            winit::keyboard::NamedKey::ChannelDown => magma_input::keyboard::Key::ChannelDown,
            winit::keyboard::NamedKey::ChannelUp => magma_input::keyboard::Key::ChannelUp,
            winit::keyboard::NamedKey::Close => magma_input::keyboard::Key::Close,
            winit::keyboard::NamedKey::MailForward => magma_input::keyboard::Key::MailForward,
            winit::keyboard::NamedKey::MailReply => magma_input::keyboard::Key::MailReply,
            winit::keyboard::NamedKey::MailSend => magma_input::keyboard::Key::MailSend,
            winit::keyboard::NamedKey::MediaClose => magma_input::keyboard::Key::MediaClose,
            winit::keyboard::NamedKey::MediaFastForward => {
                magma_input::keyboard::Key::MediaFastForward
            }
            winit::keyboard::NamedKey::MediaPause => magma_input::keyboard::Key::MediaPause,
            winit::keyboard::NamedKey::MediaPlay => magma_input::keyboard::Key::MediaPlay,
            winit::keyboard::NamedKey::MediaPlayPause => magma_input::keyboard::Key::MediaPlayPause,
            winit::keyboard::NamedKey::MediaRecord => magma_input::keyboard::Key::MediaRecord,
            winit::keyboard::NamedKey::MediaRewind => magma_input::keyboard::Key::MediaRewind,
            winit::keyboard::NamedKey::MediaStop => magma_input::keyboard::Key::MediaStop,
            winit::keyboard::NamedKey::MediaTrackNext => magma_input::keyboard::Key::MediaTrackNext,
            ...
            winit::keyboard::NamedKey::SpellCheck => magma_input::keyboard::Key::SpellCheck,
            ...
            winit::keyboard::NamedKey::AudioFaderRear => magma_input::keyboard::Key::AudioFaderRear,
            ...
            winit::keyboard::NamedKey::AudioTrebleUp => magma_input::keyboard::Key::AudioTrebleUp,
            winit::keyboard::NamedKey::AudioVolumeDown => {
                magma_input::keyboard::Key::AudioVolumeDown
            }
            winit::keyboard::NamedKey::AudioVolumeUp => magma_input::keyboard::Key::AudioVolumeUp,
            ...
            winit::keyboard::NamedKey::LaunchCalendar => magma_input::keyboard::Key::LaunchCalendar,
            winit::keyboard::NamedKey::LaunchContacts => magma_input::keyboard::Key::LaunchContacts,
            winit::keyboard::NamedKey::LaunchMail => magma_input::keyboard::Key::LaunchMail,
            ...
            winit::keyboard::NamedKey::LaunchPhone => magma_input::keyboard::Key::LaunchPhone,
            ...
            winit::keyboard::NamedKey::LaunchWebCam => magma_input::keyboard::Key::LaunchWebCam,
            winit::keyboard::NamedKey::LaunchWordProcessor => {
                magma_input::keyboard::Key::LaunchWordProcessor
            }
            winit::keyboard::NamedKey::BrowserBack => magma_input::keyboard::Key::BrowserBack,
            winit::keyboard::NamedKey::BrowserFavorites => {
                magma_input::keyboard::Key::BrowserFavorites
            }
            winit::keyboard::NamedKey::BrowserForward => magma_input::keyboard::Key::BrowserForward,
            winit::keyboard::NamedKey::BrowserHome => magma_input::keyboard::Key::BrowserHome,
            winit::keyboard::NamedKey::BrowserRefresh => magma_input::keyboard::Key::BrowserRefresh,
            winit::keyboard::NamedKey::BrowserSearch => magma_input::keyboard::Key::BrowserSearch,
            winit::keyboard::NamedKey::BrowserStop => magma_input::keyboard::Key::BrowserStop,
            winit::keyboard::NamedKey::AppSwitch => magma_input::keyboard::Key::AppSwitch,
            winit::keyboard::NamedKey::Call => magma_input::keyboard::Key::Call,
            winit::keyboard::NamedKey::Camera => magma_input::keyboard::Key::Camera,
            winit::keyboard::NamedKey::CameraFocus => magma_input::keyboard::Key::CameraFocus,
            winit::keyboard::NamedKey::EndCall => magma_input::keyboard::Key::EndCall,
            ...
            winit::keyboard::NamedKey::HeadsetHook => magma_input::keyboard::Key::HeadsetHook,
            winit::keyboard::NamedKey::LastNumberRedial => {
                magma_input::keyboard::Key::LastNumberRedial
            }
            winit::keyboard::NamedKey::Notification => magma_input::keyboard::Key::Notification,
            winit::keyboard::NamedKey::MannerMode => magma_input::keyboard::Key::MannerMode,
            winit::keyboard::NamedKey::VoiceDial => magma_input::keyboard::Key::VoiceDial,
            winit::keyboard::NamedKey::TV => magma_input::keyboard::Key::TV,
            winit::keyboard::NamedKey::TV3DMode => magma_input::keyboard::Key::TV3DMode,
            winit::keyboard::NamedKey::TVAntennaCable => magma_input::keyboard::Key::TVAntennaCable,
            ...
            winit::keyboard::NamedKey::TVContentsMenu => magma_input::keyboard::Key::TVContentsMenu,
            winit::keyboard::NamedKey::TVDataService => magma_input::keyboard::Key::TVDataService,
            winit::keyboard::NamedKey::TVInput => magma_input::keyboard::Key::TVInput,
            ...
            winit::keyboard::NamedKey::TVInputHDMI1 => magma_input::keyboard::Key::TVInputHDMI1,
            winit::keyboard::NamedKey::TVInputHDMI2 => magma_input::keyboard::Key::TVInputHDMI2,
            winit::keyboard::NamedKey::TVInputHDMI3 => magma_input::keyboard::Key::TVInputHDMI3,
            winit::keyboard::NamedKey::TVInputHDMI4 => magma_input::keyboard::Key::TVInputHDMI4,
            winit::keyboard::NamedKey::TVInputVGA1 => magma_input::keyboard::Key::TVInputVGA1,
            winit::keyboard::NamedKey::TVMediaContext => magma_input::keyboard::Key::TVMediaContext,
            winit::keyboard::NamedKey::TVNetwork => magma_input::keyboard::Key::TVNetwork,
            winit::keyboard::NamedKey::TVNumberEntry => magma_input::keyboard::Key::TVNumberEntry,
            winit::keyboard::NamedKey::TVPower => magma_input::keyboard::Key::TVPower,
            winit::keyboard::NamedKey::TVRadioService => magma_input::keyboard::Key::TVRadioService,
            winit::keyboard::NamedKey::TVSatellite => magma_input::keyboard::Key::TVSatellite,
            winit::keyboard::NamedKey::TVSatelliteBS => magma_input::keyboard::Key::TVSatelliteBS,
            winit::keyboard::NamedKey::TVSatelliteCS => magma_input::keyboard::Key::TVSatelliteCS,
            ...
            winit::keyboard::NamedKey::TVTimer => magma_input::keyboard::Key::TVTimer,
            winit::keyboard::NamedKey::AVRInput => magma_input::keyboard::Key::AVRInput,
            winit::keyboard::NamedKey::AVRPower => magma_input::keyboard::Key::AVRPower,
            winit::keyboard::NamedKey::ColorF0Red => magma_input::keyboard::Key::ColorF0Red,
            winit::keyboard::NamedKey::ColorF1Green => magma_input::keyboard::Key::ColorF1Green,
            winit::keyboard::NamedKey::ColorF2Yellow => magma_input::keyboard::Key::ColorF2Yellow,
            winit::keyboard::NamedKey::ColorF3Blue => magma_input::keyboard::Key::ColorF3Blue,
            winit::keyboard::NamedKey::ColorF4Grey => magma_input::keyboard::Key::ColorF4Grey,
            winit::keyboard::NamedKey::ColorF5Brown => magma_input::keyboard::Key::ColorF5Brown,
            ...
            winit::keyboard::NamedKey::DisplaySwap => magma_input::keyboard::Key::DisplaySwap,
            winit::keyboard::NamedKey::DVR => magma_input::keyboard::Key::DVR,
            winit::keyboard::NamedKey::Exit => magma_input::keyboard::Key::Exit,
            winit::keyboard::NamedKey::FavoriteClear0 => magma_input::keyboard::Key::FavoriteClear0,
            winit::keyboard::NamedKey::FavoriteClear1 => magma_input::keyboard::Key::FavoriteClear1,
            winit::keyboard::NamedKey::FavoriteClear2 => magma_input::keyboard::Key::FavoriteClear2,
            winit::keyboard::NamedKey::FavoriteClear3 => magma_input::keyboard::Key::FavoriteClear3,
            ...
            winit::keyboard::NamedKey::FavoriteStore0 => magma_input::keyboard::Key::FavoriteStore0,
            winit::keyboard::NamedKey::FavoriteStore1 => magma_input::keyboard::Key::FavoriteStore1,
            winit::keyboard::NamedKey::FavoriteStore2 => magma_input::keyboard::Key::FavoriteStore2,
            winit::keyboard::NamedKey::FavoriteStore3 => magma_input::keyboard::Key::FavoriteStore3,
            winit::keyboard::NamedKey::Guide => magma_input::keyboard::Key::Guide,
            winit::keyboard::NamedKey::GuideNextDay => magma_input::keyboard::Key::GuideNextDay,
            ...
            winit::keyboard::NamedKey::InstantReplay => magma_input::keyboard::Key::InstantReplay,
            winit::keyboard::NamedKey::Link => magma_input::keyboard::Key::Link,
            winit::keyboard::NamedKey::ListProgram => magma_input::keyboard::Key::ListProgram,
            winit::keyboard::NamedKey::LiveContent => magma_input::keyboard::Key::LiveContent,
            winit::keyboard::NamedKey::Lock => magma_input::keyboard::Key::Lock,
            winit::keyboard::NamedKey::MediaApps => magma_input::keyboard::Key::MediaApps,
            winit::keyboard::NamedKey::MediaAudioTrack => {
                magma_input::keyboard::Key::MediaAudioTrack
            }
            winit::keyboard::NamedKey::MediaLast => magma_input::keyboard::Key::MediaLast,
            ...
            winit::keyboard::NamedKey::MediaTopMenu => magma_input::keyboard::Key::MediaTopMenu,
            winit::keyboard::NamedKey::NavigateIn => magma_input::keyboard::Key::NavigateIn,
            winit::keyboard::NamedKey::NavigateNext => magma_input::keyboard::Key::NavigateNext,
            winit::keyboard::NamedKey::NavigateOut => magma_input::keyboard::Key::NavigateOut,
            ...
            winit::keyboard::NamedKey::OnDemand => magma_input::keyboard::Key::OnDemand,
            winit::keyboard::NamedKey::Pairing => magma_input::keyboard::Key::Pairing,
            winit::keyboard::NamedKey::PinPDown => magma_input::keyboard::Key::PinPDown,
            winit::keyboard::NamedKey::PinPMove => magma_input::keyboard::Key::PinPMove,
            winit::keyboard::NamedKey::PinPToggle => magma_input::keyboard::Key::PinPToggle,
            winit::keyboard::NamedKey::PinPUp => magma_input::keyboard::Key::PinPUp,
            winit::keyboard::NamedKey::PlaySpeedDown => magma_input::keyboard::Key::PlaySpeedDown,
            winit::keyboard::NamedKey::PlaySpeedReset => magma_input::keyboard::Key::PlaySpeedReset,
            winit::keyboard::NamedKey::PlaySpeedUp => magma_input::keyboard::Key::PlaySpeedUp,
            winit::keyboard::NamedKey::RandomToggle => magma_input::keyboard::Key::RandomToggle,
            winit::keyboard::NamedKey::RcLowBattery => magma_input::keyboard::Key::RcLowBattery,
            winit::keyboard::NamedKey::RecordSpeedNext => {
                magma_input::keyboard::Key::RecordSpeedNext
            }
            winit::keyboard::NamedKey::RfBypass => magma_input::keyboard::Key::RfBypass,
            winit::keyboard::NamedKey::ScanChannelsToggle => {
                magma_input::keyboard::Key::ScanChannelsToggle
            }
            winit::keyboard::NamedKey::ScreenModeNext => magma_input::keyboard::Key::ScreenModeNext,
            winit::keyboard::NamedKey::Settings => magma_input::keyboard::Key::Settings,
            winit::keyboard::NamedKey::SplitScreenToggle => {
                magma_input::keyboard::Key::SplitScreenToggle
            }
            winit::keyboard::NamedKey::STBInput => magma_input::keyboard::Key::STBInput,
            winit::keyboard::NamedKey::STBPower => magma_input::keyboard::Key::STBPower,
            winit::keyboard::NamedKey::Subtitle => magma_input::keyboard::Key::Subtitle,
            winit::keyboard::NamedKey::Teletext => magma_input::keyboard::Key::Teletext,
            winit::keyboard::NamedKey::VideoModeNext => magma_input::keyboard::Key::VideoModeNext,
            winit::keyboard::NamedKey::Wink => magma_input::keyboard::Key::Wink,
            winit::keyboard::NamedKey::ZoomToggle => magma_input::keyboard::Key::ZoomToggle,
        ...
        winit::keyboard::Key::Unidentified(_) => magma_input::keyboard::Key::Unidentified,
